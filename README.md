...
15. [X] 이벤트 기반 프로그래밍
16. [ ] 엔티티 모델링 설계도 추가하기
17. [ ] 물리적 모델링 객체 지향 관점으로 변경하기 (엔티티 관계, 스테레오 타입 설계 변경, 상속관계)
18. [ ] Entity를 return 하던 것 전부 DTO로 변경하기 (버전관리, 순환참조 문제 해결하기 위함)
19. [ ] 채팅 기능(웹소켓)을, event 를 사용해서 이벤트 프로그래밍 방식으로 변경하기
20. [ ] service에 존재하는 비지니스 로직들을 domain 으로 이동시키기
21. [ ] 임베디드 값 타입에 적절한 비지니스 메소드 설계 & 추가하기
22. [ ] 상속 관계에 맞춰서 엔티티를 재 설계 => 상속관계를 사용할 만한 곳이 없다면, 시스템 확장하기
23. [ ] @MappedSupperclass & 상속을 통해 기본 필드 관리하기 
24. [ ] 적절하게 배치 사이즈 조절해서 최적화
25. [ ] 패치 조인이 필요한 부분에 패치 조인 사용하기
